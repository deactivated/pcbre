...
class test_geom(unittest.TestCase):
    ...
    def test_via_via(self):
        ...
        self.assertAlmostEqual((12 ** 2 + 7 ** 2) **
                               0.5 - 10, dist_via_via(v, v1))
...
class test_aw_queries(unittest.TestCase):
    ...
    def test_trace_intersect(self):
        t = Trace(Point2(3, 3), Point2(11, 11), 1,
                  self.world.stackup.layers[0], None)
    ...
    def test_via_trace_intersect(self):
        t = Trace(Point2(3, 3), Point2(11, 11),
                  3, self.world.stackup.layers[0])
    ...
    def test_connected_island(self):
        self.world.artwork.merge_artwork(
            Trace(Point2(3, 3), Point2(11, 11), 3, self.world.stackup.layers[0]))
        connected = self.world.artwork.compute_connected(
            self.world.artwork.get_all_artwork())
        self.assertEqual(len(connected), 2)
...
class test_sequence(unittest.TestCase):
    ...
    def test_point_insert(self):
        ...
        # via should then have no net, and the traces on each layer should
        # split off
